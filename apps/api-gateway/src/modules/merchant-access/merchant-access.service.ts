...
export class MerchantAccessService {
  ...
  private async enforce(params: { actorUserId: string; principalId: string; action: string; resourceId?: string }) {
    ...
    const decision = this.policyService.evaluate(
      ...
      params.action,
      { type: 'merchant', id: params.resourceId, attributes: { principalId: params.principalId } },
      {},
    );
  ...
  async setTillBindings(params: { principalId: string; actorUserId: string; membershipId: string; tillIds: string[] }) {
    await this.enforce({
      actorUserId: params.actorUserId,
      principalId: params.principalId,
      action: 'merchant.user.manage',
    ...
    await query('DELETE FROM membership_tills WHERE membership_id = $1', [params.membershipId]);
    for (const tillId of params.tillIds) {
      await query('INSERT INTO membership_tills (membership_id, till_id) VALUES ($1, $2)', [params.membershipId, tillId]);
    }
    return { membershipId: params.membershipId, tillIds: params.tillIds };
  }
